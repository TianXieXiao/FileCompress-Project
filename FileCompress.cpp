...
FileCompress::FileCompress()
{
	...
	{
		fileByteInfo[i].ch = i;
		//有了ByteInfo构造函数fileByteInfo[i].appearCount = 0;就不需要了
	}
}
...
bool FileCompress::CompressFile(const string& filePath)
{
	...
	//文件大小暂时不知道----此处需要采用死循环的方式来获取源文件中内容
	uch readBuff[1024];
	while (true)
	{
		...
		{
			//利用到了哈希里面的直接定址法--以字符的ASCII值作为数组的下标来进行快速的统计
	...
	while (true)
	{
		...
		for (size_t i = 0; i < rdsize; i++)
		{
			...
			for (size_t j = 0; j < strCode.size(); j++)
			{
				...
				//当ch中的8个Bit位填充满之后，需要将该字节写入到压缩文件当中
				bitCount++;
				if (8 == bitCount)
				{
					fputc(ch, fOut);
...
void FileCompress::GenerateHuffmanCode(HuffmanTreeNode<ByteInfo>* root)
{
	...
	//因为在Huffman树中，所有有效权值的位置在叶子结点
	//当遍历到叶子结点的位置时，该权值对应的编码就拿到了
