...
#include <queue>
#include <vector>
//#include <functional>,没有用到greater<Node>方法，此处不需要包含该头文件
using namespace std;
...
public:
	...
	void CreateHuffmanTree(const W array[], size_t size, const W& invalid)
	{
		...
		while (q.size() > 1)
		{
			...
			//将left和right作为某个新结点的左右子树，构造一颗新的二叉树。
			//新二叉树根结点的权值就是其左右孩子权值之和
